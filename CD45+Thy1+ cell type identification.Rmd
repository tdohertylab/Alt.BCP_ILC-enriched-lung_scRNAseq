...
primary_cluster <- DimPlot(Sen_2, label = T, repel = T) +
...
```{r T cell markers, fig.width = 10, fig.height = 2}
# T cell markers
VlnPlot(Thy1, features = c("Cd3e", "Trac", "Trgc1"), assay ="RNA", pt.size = 0) & NoLegend() & theme(axis.title.x = element_blank(), axis.text.x = element_text(angle = 0, hjust = 0.5))
```

```{r T cell differential markers, fig.width = 7, fig.height = 2}
# T cell markers
...
DimPlot(ILC, label = T, repel = T) + labs(title = "ILC")
```


# ILC gene expression

```{r ILC gene markers, fig.width = 7, fig.height = 2}
# ILC markers
...
# Conventional ST-2+ IL-18R- ILC2 (nILC2): Gata3hi Rora+
# Areg ILC2 (cluster 0)
...
# ST-2 NK: Eomes+ Tbx21+ Klrb1c+ Ncr1+ Prf1+ Gata3+ Il1rl1+ (cluster 11)
...
DimPlot(Thy1, split.by = "treatment", label = F) +
...
new.thy1.numbered.ids <- c("0_Areg ILC2", "1_IL-13 ILC2", "2_IL-10 ILC2", "3_Quiescent ILC2", "4_KLRG1 ILC2", "5_LTB ILC2", "6_T", "7_NK", "8_ILC1", "9_T", "10_HSC", "11_ST-2 NK", "12_T")
...
new.thy1.ids <- c("Areg ILC2", "IL-13 ILC2", "IL-10 ILC2", "Quiescent ILC2", "KLRG1 ILC2", "LTB ILC2", "Tcm", "NK", "ILC1", "Teff", "HSC", "ST-2 NK", "Tem")
...
# Reveal the distributional change in ILC cells among the conditions with numbered clusters
ILC.numbered.distribution <- data.frame(ILC@active.ident, ILC@meta.data$treatment) 
colnames(ILC.numbered.distribution) <- c("seurat_cluster_Cell_Type", "Treatment")
ILC.numbered.distribution <- ILC.numbered.distribution %>% group_by(seurat_cluster_Cell_Type, Treatment) %>% 
  summarise(Nb = n()) %>%
  mutate(C = sum(Nb)) %>%
  mutate(Percent = Nb/C*100) 

ggplot(ILC.numbered.distribution, aes(fill = Treatment, y = Percent, x = seurat_cluster_Cell_Type)) + 
...
ggplot(ILC.distribution, aes(fill = Treatment, y = Percent, x = Cell_Type)) + 
  geom_bar(position="fill", stat="identity") + xlab("") + ylab("Proportion") + xtheme + 
  theme(legend.position='right', axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1)) +
  theme(legend.title = element_text(size = 14, face = "bold"), legend.text = element_text(size = 14)) +
  ggtitle("ILC distribution across the treatments") +
...
```{r ILC cell distribution for paper}
...
```{r Labeled ILC differential gene expression, fig.width = 7, fig.height = 3}
ILC.subs <- c("Ltb", "Lta", "Tnf", "Il22", "Il17a", "Rorc", "Il13", "Il10", "Il5", "Arg1", "Klrg1", "Il1rl1", "Gata3", "Rora", "Itga1", "Itga2", "Gzma", "Prf1", "Ifng", "Ncr1", "Klrc2", "Eomes", "Tbx21")

DotPlot(ILC, features = rev(ILC.subs), group.by = "cell_type", assay = "RNA", cols = "RdBu") + theme(axis.title = element_blank(), legend.title = element_text(size = 8, face ="bold"), legend.text = element_text(size = 8), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))

DotPlot(ILC, features = rev(ILC.subs), group.by = "seurat_cluster_cell_type", assay = "RNA", cols = "RdBu") + theme(axis.title = element_blank(), legend.title = element_text(size = 8, face ="bold"), legend.text = element_text(size = 8), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))


DotPlot(ILC, features = rev(ATF), group.by = "cell_type", assay = "RNA", cols = "RdBu") + theme(axis.title = element_blank(), legend.title = element_text(size = 8, face ="bold"), legend.text = element_text(size = 8), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))

DotPlot(ILC, features = rev(ATF), group.by = "seurat_cluster_cell_type", assay = "RNA", cols = "RdBu") + theme(axis.title = element_blank(), legend.title = element_text(size = 8, face ="bold"), legend.text = element_text(size = 8), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))
```
...
```{r ILC cytokine genes, fig.width = 8, fig.height = 5}
# Cytokines
cytokines <- c("Prf1", "Gzma", "Gzmb", "Ifng", "Areg", "Il2", "Il4", "Il5", "Il6", "Il10", "Il13", "Il17a", "Il22", "Csf2", "Tnf", "Lta", "Ltb", "Tnfsf10", "Tnfsf11", "Tnfsf14", "Tgfb1")

DotPlot(ILC, features = rev(cytokines), group.by = "cell_type", split.by = "treatment", assay = "RNA", cols = "RdBu") + theme(axis.title = element_blank(), legend.title = element_text(size = 8, face ="bold"), legend.text = element_text(size = 8), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))

DotPlot(ILC, features = rev(cytokines), group.by = "seurat_cluster_cell_type", split.by = "treatment", assay = "RNA", cols = "RdBu") + theme(axis.title = element_blank(), legend.title = element_text(size = 8, face ="bold"), legend.text = element_text(size = 8), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))
```

```{r ILC chemokine genes, fig.width = 5, fig.height = 5}
# Chemokines
chemokines <- c("Ccl1", "Ccl2", "Ccl3", "Ccl3l1", "Ccl4", "Ccl4l1", "Ccl4l2", "Ccl5", "Cxcl2", "Cxcl4", "Cxcl4l1", "Cxcl9", "Cxcl10", "Cxcl11", "Cxcl12")

DotPlot(ILC, features = rev(chemokines), group.by = "cell_type", split.by = "treatment", assay = "RNA", cols = "RdBu") + theme(axis.title = element_blank(), legend.title = element_text(size = 8, face ="bold"), legend.text = element_text(size = 8), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))

DotPlot(ILC, features = rev(chemokines), group.by = "seurat_cluster_cell_type", split.by = "treatment", assay = "RNA", cols = "RdBu") + theme(axis.title = element_blank(), legend.title = element_text(size = 8, face ="bold"), legend.text = element_text(size = 8), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))
```

```{r ILC activation transcription factor genes, fig.width = 8, fig.height = 5}
# Activation transcription factors
DotPlot(ILC, features = rev(ATF), group.by = "cell_type", split.by = "treatment", assay = "RNA", cols = "RdBu") + theme(axis.title = element_blank(), legend.title = element_text(size = 8, face ="bold"), legend.text = element_text(size = 8), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))

DotPlot(ILC, features = rev(ATF), group.by = "seurat_cluster_cell_type", split.by = "treatment", assay = "RNA", cols = "RdBu") + theme(axis.title = element_blank(), legend.title = element_text(size = 8, face ="bold"), legend.text = element_text(size = 8), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))
```

```{r ILC effector genes for paper, fig.width = 12, fig.height = 6}
# ILC cytokine and chemokine
...
T1_cytokine <- c("Ifng")
T2_cytokine <- c("Il5", "Il13")
Neutrophilia <- c("Csf2", "Cxcl2")
Remodeling <- c("Areg", "Tgfb1")

Effector_markers <- list("Remodel" = Remodeling, "Neu" = Neutrophilia, "Type 2" = T2_cytokine, "Type 1" = T1_cytokine)

Effector_function <- DotPlot(ILC, features = rev(Effector_markers), group.by = "treatment", assay = "RNA", cols = "RdBu") + theme(axis.title = element_blank(), legend.title = element_text(size = 8, face ="bold"), legend.text = element_text(size = 8), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) +
  coord_flip() +
  ggtitle("Ovarall effector function")
...
```{r Paper figure 3 layout, fig.width = 14, fig.height = 18}
# Generate the plot in paper figure form
figure3_1 <- plot_grid(ILC_differential, labels = c('(A)'), label_size = 18)
figure3_2 <- plot_grid(ILC_cluster, NULL, ILC_distribution, labels = c('(B)', '(C)'), label_size = 18, nrow = 1, rel_widths = c(1.2, 0.1, 1))
figure3_3 <- plot_grid(ILC_Cyto_Chemo, labels = c('(D)'), label_size = 18)
figure3_4 <- plot_grid(ILC_ATF, Effector_function, labels = c('(E)', '(F)'), label_size = 18, nrow = 1, rel_widths = c(1.7, 1))

figure3 <- plot_grid(figure3_1, figure3_2, figure3_3, figure3_4,
  nrow = 4,
  ncol = 1,
  rel_heights = c(1, 1.2, 1.6, 1.6)
)

figure3
```

```{r Paper figure S1 layout, fig.width = 12, fig.height = 12}
# Generate the plot in paper figure form
figureS1 <- plot_grid(markers_heatmap,
...
ggsave2(plot = figure3, path = "./Figures", "Figure3.tiff", device = "tiff", dpi = 300, bg = "white", width = 14, height = 18, units = "in")

ggsave2(plot = figureS1, path = "./Figures", "FigureS1.tiff", device = "tiff", dpi = 300, bg = "white", width = 12, height = 12, units = "in")
```
