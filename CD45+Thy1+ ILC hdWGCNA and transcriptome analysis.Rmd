---
title: "CD45+Thy1+ ILC hdWGCNA and transcriptome analysis"
output:
   html_document: 
     theme: united
     df_print: kable
     toc: yes
date: 'Compiled: `r format(Sys.Date(), "%B %d, %Y")`'
---

```{r setup, include = FALSE}
all_times <- list()  # store the time for each chunk
knitr::knit_hooks$set(time_it = local({
  now <- NULL
  function(before, options) {
    if (before) {
      now <<- Sys.time()
    } else {
      res <- difftime(Sys.time(), now, units = "secs")
      all_times[[options$label]] <<- res
    }
  }
}))
knitr::opts_chunk$set(
  tidy = TRUE,
  tidy.opts = list(width.cutoff = 95),
  message = FALSE,
  warning = FALSE,
  time_it = TRUE
)
```


# Universal packages

```{r Load packages}
library(Seurat)
library(tidyverse)
library(patchwork)
library(cowplot)
```


# Data input

```{r Load SCT processed data}
# Load CD45+Thy1+ data

# Thy1+
Thy1 <- readRDS("CD45_Thy1_DP_labeled_paper_ver.rds")


# Set RNA assay as default
DefaultAssay(Thy1) <- "RNA"
head(x = Thy1[[]])
```


# CD45+ Thy1+ cluster Visualization

```{r Set Reveal CD45+Thy1+ clusters}
# Show the numbers for CD45+Thy1+
table(Thy1@meta.data$cell_type, split.by = Thy1@meta.data$treatment)

# Plot the CD45+ Thy1+ clusters
DimPlot(Thy1, label = T, repel =  T)
DimPlot(Thy1, group.by = "treatment", label = F) + ggtitle(NULL)
```


# Overall Thy1 subsets

```{r Define cell subsets}
# Label the overall annotation
overall.ids.CNA <- c("ILC2", "ILC2", "ILC2", "ILC2", "ILC2", "ILC2", "T", "NK", "ILC1", "T", "HSC", "NK", "T")
names(overall.ids.CNA) <- levels(Thy1)
Thy1 <- RenameIdents(Thy1, overall.ids.CNA)
Thy1 <- AddMetaData(Thy1, metadata = Thy1@active.ident, col.name = 'cell_type_overall')

# Show the numbers across the conditions
table(Thy1@active.ident, split.by = Thy1@meta.data$treatment)

# Plot the CD45+Thy1+ clusters
DimPlot(Thy1, label = T, repel =  T)
DimPlot(Thy1, group.by = "treatment", label = F) + ggtitle(NULL)
```

```{r Set bqck primary ident}
# Set the primary ident to "cell_type"
Thy1 <- SetIdent(Thy1, value = "cell_type")
```


# Packages for hdWGCNA and GSEA

```{r Prepare the packages for hdWGCNA}
# Load co-expression network analysis packages
library(WGCNA)
library(hdWGCNA)

# Load network analysis & visualization package
library(igraph)
```

```{r Prepare the packages for GSE and pathway analysis}
# Load packages for enrichment analysis (enrichR)
library(enrichR)

# Check available databases to perform enrichment (then choose one)
enrichR::listEnrichrDbs()
```

```{r Prepare the packages for GeneOverlap}
# Marker gene overlap analysis
library(GeneOverlap)
```


# Preparation of co-expression network analysis (CNA) on overall ILC types

```{r Prepare to run overall ILC coexpression network}
# Set up Seurat object for WGCNA for overall ILC
Thy1 <- SetupForWGCNA(Thy1,
  gene_select = "fraction",
  fraction = 0.05, 
  # fraction of cells that a gene needs to be expressed in order to be included
  wgcna_name = "Overall ILC hdWGCNA" 
  # the name of the hdWGCNA experiment
)

# Construct metacells  in each group
Thy1 <- MetacellsByGroups(
  Thy1,
  group.by = c("cell_type_overall", "treatment"),
  # specify the columns in seurat_obj@meta.data to group by
  reduction = 'umap',
  # select the dimensionality reduction to perform KNN on
  assay = 'RNA',
  slot = 'counts',
  k = 25,
  # nearest-neighbors parameter
  max_shared = 10,
  # maximum number of shared cells between two metacells
  ident.group = 'cell_type_overall'
  # set the Idents of the metacell seurat object
)

# Normalize metacell expression matrix
Thy1 <- NormalizeMetacells(Thy1)
```


# ILC Alt.BPC hdWGCNA

```{r Run ILC_BPC coexpression network}
# Set up the expression matrix on ILC
ILC.BPC_CNA <- SetDatExpr(
  Thy1,
  group.by='cell_type_overall',
  # the metadata column containing the cell type info. This same column should have also been used in MetacellsByGroups
  group_name = c("NK", "ILC1", "ILC2"),
  # the name of the group of interest in the group.by column
  multi.group.by = 'treatment',
  #A string containing the name of a column in the Seurat object with groups for consensus WGCNA (dataset, sample, condition, etc)
  multi_group_name = c("Alt.BPC"),
  #A string containing the name of a group present in the multi.group.by column
  assay = 'RNA',
  # using RNA assay
  slot = 'data'
  # using normalized data
)

# Test different soft powers
ILC.BPC_CNA <- TestSoftPowers(
  ILC.BPC_CNA,
  networkType = 'signed'
  # "unsigned" or "signed hybrid" can also be used
)

# Plot the results
plot_list_ILC <- PlotSoftPowers(ILC.BPC_CNA)

# Reveal the data
wrap_plots(plot_list_ILC, ncol = 2)

power_table_ILC <- GetPowerTable(ILC.BPC_CNA)
head(power_table_ILC)

# Construct co-expression network
ILC.BPC_CNA <- ConstructNetwork(
  ILC.BPC_CNA,
  setDatExpr = FALSE,
  overwrite_tom = TRUE,
  tom_name = 'ILC_BPC-log'
  # name of the topoligical overlap matrix written to disk
)

PlotDendrogram(ILC.BPC_CNA, main='ILC Alt.BPC hdWGCNA Dendrogram')

# Need to run ScaleData first or else harmony throws an error
ILC.BPC_CNA <- ScaleData(ILC.BPC_CNA, features=VariableFeatures(ILC.BPC_CNA))

# Compute all MEs in the full single-cell dataset
ILC.BPC_CNA <- ModuleEigengenes(ILC.BPC_CNA)

# Compute eigengene-based connectivity (kME)
ILC.BPC_CNA <- ModuleConnectivity(ILC.BPC_CNA)

# Rename the modules
ILC.BPC_CNA <- ResetModuleNames(
  ILC.BPC_CNA,
  new_name = "M"
)

# Get the module assignment table:
ILC.BPC_modules <- GetModules(ILC.BPC_CNA)

# show the first 10 columns:
head(ILC.BPC_modules[,1:10])
```

```{r Plot ILC_BPC coexpression network, fig.height = 4, fig.width = 10}
# Plot genes raILCed by kME for each module
PlotKMEs(ILC.BPC_CNA, ncol = 5)
```

```{r Change the colors of selected modules}
# Set new colors
new_module_colors <- list('M5' = 'darksalmon')
ILC.BPC_CNA <- ResetModuleColors(ILC.BPC_CNA, new_module_colors )
```

```{r Plot ILC_BPC hub gene network, fig.height = 10, fig.width = 10}
# Plot hubgene network
HubGeneNetworkPlot(
  ILC.BPC_CNA,
  n_hubs = 6, n_other = 0,
  edge_prop = 1,
  edge.alpha = 0.3,
  vertex.label.cex = 1,
  hub.vertex.size = 5,
  other.vertex.size = 0,
  mods = 'all'
)
```

```{r ILC_BPC Enrichment analysis}
# Perform enrichment tests
ILC.BPC_CNA <- RunEnrichr(
  ILC.BPC_CNA,
  dbs = "WikiPathways_2019_Mouse",
  # character vector of enrichr databases to test
  max_genes = 100 
  # number of genes per module to test. use max_genes = Inf to choose all genes!
)

# Retrieve the output table
GetEnrichrTable(ILC.BPC_CNA)
```

```{r ILC_BPC EnrichrDotPlot for paper, fig.width = 7, fig.height = 5}
# Enrichr dotplot
ILC.EnrichrDotPlot <- EnrichrDotPlot(
  ILC.BPC_CNA,
  mods = "all",
  # use all modules (this is the default behavior)
  database = "WikiPathways_2019_Mouse",
  # this has to be one of the lists we used above!!!
  n_terms=2 # number of terms for each module
) + theme(axis.text.x = element_text(size = 10, angle = 0, vjust = 0.5, hjust = 0.5)) +
  ggtitle("Enrichment Analysis")

ILC.EnrichrDotPlot
```

```{r Rename the modules for paper, fig.height = 7, fig.width = 14}
# Rename the modules
rename_list <- list(
  "M1" = "M1: TYROBP causal Network",
  "M2" = "M2: DNA Replication",
  "M3" = "M3: MAPK signaling pathway",
  "M4" = "M4: Type II interferon signaling (IFNG)",
  "M5" = "M5: Cytoplasmic Ribosomal Proteins",
  "M6" = "M6: p53 Signaling",
  "M7" = "M7: Neuclear Receptors",
  "M8" = "M8: Electron Transport Chain",
  "M9" = "M9: Cytoplasmic Ribosomal Proteins"
)

ILC.BPC_CNA_renamed <- ResetModuleNames(
  ILC.BPC_CNA,
  new_name = rename_list
)

# Plot genes raILCed by kME for each module
ILC.PlotKMEs <- PlotKMEs(ILC.BPC_CNA_renamed,
                         ncol = 3,
                         text_size = 3,
                         plot_widths = c(1, 3)
                         )

ILC.PlotKMEs

# Get the module assignment table:
modules <- GetModules(ILC.BPC_CNA_renamed)

# Show the first 4 columns:
head(modules[,1:4])
```

```{r ILC_BPC coexpression dotplot, fig.width = 5.5, fig.height = 6}
# Compute gene scoring for the top 25 hub genes by kME for each module
# With Seurat method
ILC.BPC_CNA <- ModuleExprScore(
  ILC.BPC_CNA,
  n_genes = 25,
  method='Seurat'
)

# Get hMEs from seurat object
ILC.BPC_MEs <- GetMEs(ILC.BPC_CNA, harmonized=TRUE)
ILC.BPC_mods <- colnames(ILC.BPC_MEs); ILC.BPC_mods <- ILC.BPC_mods[ILC.BPC_mods != 'grey']

# Reorder the vector ILC.BPC_mods into numeric order
ILC.BPC_mods <- sort(ILC.BPC_mods)


# Add hMEs to Seurat meta-data:
ILC.BPC_CNA@meta.data <- cbind(ILC.BPC_CNA@meta.data, ILC.BPC_MEs)

# Plot with Seurat's DotPlot function
DotPlot(ILC.BPC_CNA, features = ILC.BPC_mods, group.by = 'cell_type', split.by = 'treatment', cols = "RdBu") + theme(axis.title = element_blank(), legend.title = element_text(size = 8, face ="bold"), legend.text = element_text(size = 8), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1), axis.text.y = element_text(size = 10))
```

```{r ILC_BPC marker gene OverlapDotPlot, fig.width = 5, fig.height = 3}
# Compute marker genes in ILC with Seurat
ILC.BPC_CNA_ILC <- ILC.BPC_CNA %>%
  subset(idents = c("Tcm", "Teff", "HSC", "Tem"), invert = TRUE)

table(ILC.BPC_CNA_ILC@meta.data$cell_type, split.by = ILC.BPC_CNA_ILC@meta.data$treatment)

ILC.BPC_ILC_markers <- Seurat::FindAllMarkers(
  ILC.BPC_CNA_ILC,
  only.pos = TRUE,
  logfc.threshold=1
)

# Compute marker gene overlaps in ILC
ILC.BPC_ILC_overlap <- OverlapModulesDEGs(
  ILC.BPC_CNA_ILC,
  deg_df = ILC.BPC_ILC_markers,
  fc_cutoff = 1 # log fold change cutoff for overlap analysis
)

# Plot odds ratio of the overlap as a dot plot for ILC
ILC_OverlapDotPlot <- OverlapDotPlot(
  ILC.BPC_ILC_overlap,
  plot_var = 'odds_ratio') +
  ggtitle('Overlap of ILC_BPC modules to cell types') + 
  theme(axis.text.x = element_text(size = 10, angle = 0, vjust = 0.5, hjust = 0.5))

ILC_OverlapDotPlot
```

```{r ILC_BPC marker gene in ILC1 and NK OverlapDotPlot, fig.width = 5, fig.height = 2}
# Compute marker genes in ILC1 and NK with Seurat
ILC.BPC_CNA_ILC_NK <- ILC.BPC_CNA %>%
  subset(idents = c("NK", "ILC1", "ST-2 NK"), invert = FALSE) %>%
  SetIdent(value = "treatment")

table(ILC.BPC_CNA_ILC_NK@meta.data$cell_type_overall, split.by = ILC.BPC_CNA_ILC_NK@meta.data$treatment)

ILC.BPC_ILC1_NK_markers <- Seurat::FindAllMarkers(
  ILC.BPC_CNA_ILC_NK,
  only.pos = TRUE,
  logfc.threshold=0.5
)

# Compute marker gene overlaps in ILC1 and NK
ILC.BPC_ILC1_NK_overlap <- OverlapModulesDEGs(
  ILC.BPC_CNA_ILC_NK,
  deg_df = ILC.BPC_ILC1_NK_markers,
  fc_cutoff = 0.5 # log fold change cutoff for overlap analysis
)

# Plot odds ratio of the overlap as a dot plot for ILC1 and NK
ILC1_NK_OverlapDotPlot <- OverlapDotPlot(
  ILC.BPC_ILC1_NK_overlap,
  plot_var = 'odds_ratio') +
  ggtitle('Overlap of modules in ILC1 and NK') + 
  theme(axis.text.x = element_text(size = 10, angle = 0, vjust = 0.5, hjust = 0.5))

ILC1_NK_OverlapDotPlot
```

```{r ILC_BPC marker gene in ILC1 and NK OverlapBarPlot, fig.width = 3, fig.height = 3}
# Overlap barplot for Alt, produces a plot for each cell type
plot_list_ILC1_NK <- OverlapBarPlot(ILC.BPC_ILC1_NK_overlap)

# Stitch plots with patchwork
ILC1_NK_OverlapBarPlot <- wrap_plots(plot_list_ILC1_NK)

ILC1_NK_OverlapBarPlot.title <- plot_grid(NULL, ILC1_NK_OverlapBarPlot, labels = c('ILC1 and NK'), label_size = 12, ncol = 1, rel_heights = c(0.07, 1))

ILC1_NK_OverlapBarPlot.title
```

```{r ILC_BPC marker gene in ILC2 OverlapDotPlot, fig.width = 5, fig.height = 2}
# Compute marker genes in ILC2 with Seurat
ILC.BPC_CNA_ILC2 <- ILC.BPC_CNA %>%
  subset(idents = c("Areg ILC2", "IL-13 ILC2", "IL-10 ILC2", "Quiescent ILC2", "KLRG1 ILC2", "LTB ILC2"), invert = FALSE) %>%
  SetIdent(value = "treatment")

table(ILC.BPC_CNA_ILC2@meta.data$cell_type_overall, split.by = ILC.BPC_CNA_ILC2@meta.data$treatment)

ILC.BPC_ILC2_markers <- Seurat::FindAllMarkers(
  ILC.BPC_CNA_ILC2,
  only.pos = TRUE,
  logfc.threshold=0.5
)

# Compute marker gene overlaps in ILC2
ILC.BPC_ILC2_overlap <- OverlapModulesDEGs(
  ILC.BPC_CNA_ILC2,
  deg_df = ILC.BPC_ILC2_markers,
  fc_cutoff = 0.5 # log fold change cutoff for overlap analysis
)

# Plot odds ratio of the overlap as a dot plot for ILC2
ILC2_OverlapBarPlot <- OverlapDotPlot(
  ILC.BPC_ILC2_overlap,
  plot_var = 'odds_ratio') +
  ggtitle('Overlap of modules in ILC2') + 
  theme(axis.text.x = element_text(size = 10, angle = 0, vjust = 0.5, hjust = 0.5))

ILC2_OverlapBarPlot
```

```{r ILC_BPC marker gene in ILC2 OverlapBarPlot, fig.width = 3, fig.height = 3}
# Overlap barplot for Alt, produces a plot for each cell type
plot_list_ILC2 <- OverlapBarPlot(ILC.BPC_ILC2_overlap)

# Stitch plots with patchwork
ILC2_OverlapBarPlot <- wrap_plots(plot_list_ILC2)

ILC2_OverlapBarPlot.title <- plot_grid(NULL, ILC2_OverlapBarPlot, labels = c('    ILC2'), label_size = 12, ncol = 1, rel_heights = c(0.07, 1))

ILC2_OverlapBarPlot.title
```


# ILC_BPC module gene sets

```{r ILC_BPC module gene sets}
# ILC_BPC module genes fromhdWGCNA
ILC_BPC_M1 <- c("Tyrobp", "Plek", "Ugcg", "Gzma", "Fcer1g", "Klrd1", "Car2", "Klrk1", "Nkg7", "Ms4a4b")

ILC_BPC_M2 <- c("Birc5", "Ptma", "Rrm1", "4921515E04Rik", "Tubb5", "Rrm2", "Hmgb2", "Stmn1", "Tuba1b", "Il31ra")

ILC_BPC_M3 <- c("Lmo4", "Sik1", "Ywhaz", "Zc3hav1", "Psap", "Nr4a3", "Ddx3x", "Dusp1", "Fosb", "Nr4a1")

ILC_BPC_M4 <- c("Gbp6", "Gbp2", "Tgtp2", "Samhd1", "Zbp1", "Slfn5", "Ifit1", "Stat1", "Rsad2", "Isg15")

ILC_BPC_M5 <- c("Rp135", "Rps18", "Rps28", "Rps27", "Gm10076", "Rps19", "Rps5", "Rpl23a", "Rpl37a", "Rpl41")

ILC_BPC_M6 <- c("Sdf4", "Axl", "Timp2", "Pmepa1", "Tff1", "Tnfsf11", "Eef2", "Ndfip1", "Edn3", "AA467197")

ILC_BPC_M7 <- c("Pcsk1", "Cd82", "Cish", "Areg", "Il1rl1", "Gadd45b", "Csf2", "Bhlhe40", "Furin", "Gata3")

ILC_BPC_M8 <- c("Cox6a1", "Uqcrq", "Calm1", "Gm53028", "Uqcr10", "Atp5e", "Oaz1", "Cfap77", "Gapdh", "Lgals1")

ILC_BPC_M9 <- c("Rplp1", "Rps21", "Rps10", "Rps20", "Rpsa", "Rps27a", "Rpl38", "Rps4x", "Rps15a", "Rps12")


Module_M1_M3_M7 <- list("M1" = ILC_BPC_M1, "M3" = ILC_BPC_M3, "M7" = ILC_BPC_M7)

Module_M2_M6 <- list("M2" = ILC_BPC_M2, "M6" = ILC_BPC_M6)

Module_M5_M9 <- list("M5" = ILC_BPC_M5, "M9" = ILC_BPC_M9)


Module_M4 <- list(" " = ILC_BPC_M4)

Module_M8 <- list(" " = ILC_BPC_M8)
```


# NK/ILC1 module gene expression

```{r ILC_BPC module expression in NK_ILC1, fig.width = 5, fig.height = 3}
# Plot with Seurat's DotPlot function
NK_ILC1_module <- DotPlot(ILC.BPC_CNA, features = ILC.BPC_mods, idents = c("NK", "ILC1", "ST-2 NK"), group.by = 'cell_type', split.by = 'treatment', cols = "RdBu") + theme(axis.title = element_blank(), legend.title = element_text(size = 8, face ="bold"), legend.text = element_text(size = 8), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1), axis.text.y = element_text(size = 10, color = c("dimgray", "black"))) + ggtitle("Module expression")

NK_ILC1_module
```

```{r NK_ILC1 module expression, fig.width = 6, fig.height = 4}
# Dotplot
DotPlot(ILC.BPC_CNA, features = rev(ILC_BPC_M1), idents = c("NK", "ILC1", "ST-2 NK"), group.by = "cell_type", split.by = "treatment", assay = "RNA", cols = "RdBu") + theme(axis.title = element_blank(), legend.title = element_text(size = 8, face ="bold"), legend.text = element_text(size = 8), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) + ggtitle("M1: TYROBP causal Network")

DotPlot(ILC.BPC_CNA, features = rev(ILC_BPC_M2), idents = c("NK", "ILC1", "ST-2 NK"), group.by = "cell_type", split.by = "treatment", assay = "RNA", cols = "RdBu") + theme(axis.title = element_blank(), legend.title = element_text(size = 8, face ="bold"), legend.text = element_text(size = 8), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) + ggtitle("M2: DNA Replication")

DotPlot(ILC.BPC_CNA, features = rev(ILC_BPC_M3), idents = c("NK", "ILC1", "ST-2 NK"), group.by = "cell_type", split.by = "treatment", assay = "RNA", cols = "RdBu") + theme(axis.title = element_blank(), legend.title = element_text(size = 8, face ="bold"), legend.text = element_text(size = 8), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) + ggtitle("M3: MAPK signaling pathway")

DotPlot(ILC.BPC_CNA, features = rev(ILC_BPC_M4), idents = c("NK", "ILC1", "ST-2 NK"), group.by = "cell_type", split.by = "treatment", assay = "RNA", cols = "RdBu") + theme(axis.title = element_blank(), legend.title = element_text(size = 8, face ="bold"), legend.text = element_text(size = 8), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) + ggtitle("M4: Type II interferon signaling (IFNG)")

DotPlot(ILC.BPC_CNA, features = rev(ILC_BPC_M5), idents = c("NK", "ILC1", "ST-2 NK"), group.by = "cell_type", split.by = "treatment", assay = "RNA", cols = "RdBu") + theme(axis.title = element_blank(), legend.title = element_text(size = 8, face ="bold"), legend.text = element_text(size = 8), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) + ggtitle("M5: Cytoplasmic Ribosomal Proteins")

DotPlot(ILC.BPC_CNA, features = rev(ILC_BPC_M6), idents = c("NK", "ILC1", "ST-2 NK"), group.by = "cell_type", split.by = "treatment", assay = "RNA", cols = "RdBu") + theme(axis.title = element_blank(), legend.title = element_text(size = 8, face ="bold"), legend.text = element_text(size = 8), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) + ggtitle("M6: p53 Signaling")

DotPlot(ILC.BPC_CNA, features = rev(ILC_BPC_M7), idents = c("NK", "ILC1", "ST-2 NK"), group.by = "cell_type", split.by = "treatment", assay = "RNA", cols = "RdBu") + theme(axis.title = element_blank(), legend.title = element_text(size = 8, face ="bold"), legend.text = element_text(size = 8), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) + ggtitle("M7: Neuclear Receptors")

DotPlot(ILC.BPC_CNA, features = rev(ILC_BPC_M8), idents = c("NK", "ILC1", "ST-2 NK"), group.by = "cell_type", split.by = "treatment", assay = "RNA", cols = "RdBu") + theme(axis.title = element_blank(), legend.title = element_text(size = 8, face ="bold"), legend.text = element_text(size = 8), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) + ggtitle("M8: Electron Transport Chain")

DotPlot(ILC.BPC_CNA, features = rev(ILC_BPC_M9), idents = c("NK", "ILC1", "ST-2 NK"), group.by = "cell_type", split.by = "treatment", assay = "RNA", cols = "RdBu") + theme(axis.title = element_blank(), legend.title = element_text(size = 8, face ="bold"), legend.text = element_text(size = 8), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) + ggtitle("M9: Cytoplasmic Ribosomal Proteins")
```

```{r ILC.BPC_CNA module expression for paper, fig.width = 11, fig.height = 4}
NK_ILC1_M1_M3_M7 <- DotPlot(ILC.BPC_CNA, features = Module_M1_M3_M7, idents = c("NK", "ILC1", "ST-2 NK"), group.by = "cell_type", split.by = "treatment", assay = "RNA", cols = "RdBu") + theme(axis.title = element_blank(), legend.title = element_text(size = 8, face ="bold"), legend.text = element_text(size = 8), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1), axis.text.y = element_text(color = c("dimgray", "black"))) + ggtitle("M1: TYROBP causal Network/M3: MAPK signaling pathway/M7: Neuclear Receptors")

NK_ILC1_M1_M3_M7


NK_ILC1_M2_M6 <- DotPlot(ILC.BPC_CNA, features = Module_M2_M6, idents = c("NK", "ILC1", "ST-2 NK"), group.by = "cell_type", split.by = "treatment", assay = "RNA", cols = "RdBu") + theme(axis.title = element_blank(), legend.title = element_text(size = 8, face ="bold"), legend.text = element_text(size = 8), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1), axis.text.y = element_text(color = c("dimgray", "black"))) + ggtitle("M2: DNA Replication/M6: p53 Signaling")

NK_ILC1_M2_M6


NK_ILC1_M8 <- DotPlot(ILC.BPC_CNA, features = Module_M8, idents = c("NK", "ILC1", "ST-2 NK"), group.by = "cell_type", split.by = "treatment", assay = "RNA", cols = "RdBu") + theme(axis.title = element_blank(), legend.title = element_text(size = 8, face ="bold"), legend.text = element_text(size = 8), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1), axis.text.y = element_text(color = c("dimgray", "black"))) + ggtitle("M8: Electron Transport Chain")

NK_ILC1_M8


NK_ILC1_M5_M9 <- DotPlot(ILC.BPC_CNA, features = Module_M5_M9, idents = c("NK", "ILC1", "ST-2 NK"), group.by = "cell_type", split.by = "treatment", assay = "RNA", cols = "RdBu") + theme(axis.title = element_blank(), legend.title = element_text(size = 8, face ="bold"), legend.text = element_text(size = 8), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1), axis.text.y = element_text(color = c("dimgray", "black"))) + ggtitle("M5/M9: Cytoplasmic Ribosomal Proteins")

NK_ILC1_M5_M9


NK_ILC1_M4 <- DotPlot(ILC.BPC_CNA, features = Module_M4, idents = c("NK", "ILC1", "ST-2 NK"), group.by = "cell_type", split.by = "treatment", assay = "RNA", cols = "RdBu") + theme(axis.title = element_blank(), legend.title = element_text(size = 8, face ="bold"), legend.text = element_text(size = 8), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1), axis.text.y = element_text(color = c("dimgray", "black"))) + ggtitle("M4: Type II interferon signaling (IFNG)")

NK_ILC1_M4
```


# ILC2 module expression

```{r ILC_BPC module expression in ILC2, fig.width = 5, fig.height = 5}
# Plot with Seurat's DotPlot function
ILC2_module <- DotPlot(ILC.BPC_CNA, features = ILC.BPC_mods, idents = c("Areg ILC2", "IL-13 ILC2", "IL-10 ILC2", "Quiescent ILC2", "KLRG1 ILC2", "LTB ILC2"), group.by = 'cell_type', split.by = 'treatment', cols = "RdBu") + theme(axis.title = element_blank(), legend.title = element_text(size = 8, face ="bold"), legend.text = element_text(size = 8), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1), axis.text.y = element_text(size = 10, color = c("dimgray", "black"))) + ggtitle("Module expression")

ILC2_module
```

```{r ILC2 module expression, fig.width = 7, fig.height = 5}
# Dotplot
DotPlot(ILC.BPC_CNA, features = rev(ILC_BPC_M1), idents = c("Areg ILC2", "IL-13 ILC2", "IL-10 ILC2", "Quiescent ILC2", "KLRG1 ILC2", "LTB ILC2"), group.by = "cell_type", split.by = "treatment", assay = "RNA", cols = "RdBu") + theme(axis.title = element_blank(), legend.title = element_text(size = 8, face ="bold"), legend.text = element_text(size = 8), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) + ggtitle("M1: TYROBP causal Network")

DotPlot(ILC.BPC_CNA, features = rev(ILC_BPC_M2), idents = c("Areg ILC2", "IL-13 ILC2", "IL-10 ILC2", "Quiescent ILC2", "KLRG1 ILC2", "LTB ILC2"), group.by = "cell_type", split.by = "treatment", assay = "RNA", cols = "RdBu") + theme(axis.title = element_blank(), legend.title = element_text(size = 8, face ="bold"), legend.text = element_text(size = 8), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) + ggtitle("M2: DNA Replication")

DotPlot(ILC.BPC_CNA, features = rev(ILC_BPC_M3), idents = c("Areg ILC2", "IL-13 ILC2", "IL-10 ILC2", "Quiescent ILC2", "KLRG1 ILC2", "LTB ILC2"), group.by = "cell_type", split.by = "treatment", assay = "RNA", cols = "RdBu") + theme(axis.title = element_blank(), legend.title = element_text(size = 8, face ="bold"), legend.text = element_text(size = 8), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) + ggtitle("M3: MAPK signaling pathway")

DotPlot(ILC.BPC_CNA, features = rev(ILC_BPC_M4), idents = c("Areg ILC2", "IL-13 ILC2", "IL-10 ILC2", "Quiescent ILC2", "KLRG1 ILC2", "LTB ILC2"), group.by = "cell_type", split.by = "treatment", assay = "RNA", cols = "RdBu") + theme(axis.title = element_blank(), legend.title = element_text(size = 8, face ="bold"), legend.text = element_text(size = 8), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) + ggtitle("M4: Type II interferon signaling (IFNG)")

DotPlot(ILC.BPC_CNA, features = rev(ILC_BPC_M5), idents = c("Areg ILC2", "IL-13 ILC2", "IL-10 ILC2", "Quiescent ILC2", "KLRG1 ILC2", "LTB ILC2"), group.by = "cell_type", split.by = "treatment", assay = "RNA", cols = "RdBu") + theme(axis.title = element_blank(), legend.title = element_text(size = 8, face ="bold"), legend.text = element_text(size = 8), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) + ggtitle("M5: Cytoplasmic Ribosomal Proteins")

DotPlot(ILC.BPC_CNA, features = rev(ILC_BPC_M6), idents = c("Areg ILC2", "IL-13 ILC2", "IL-10 ILC2", "Quiescent ILC2", "KLRG1 ILC2", "LTB ILC2"), group.by = "cell_type", split.by = "treatment", assay = "RNA", cols = "RdBu") + theme(axis.title = element_blank(), legend.title = element_text(size = 8, face ="bold"), legend.text = element_text(size = 8), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) + ggtitle("M6: p53 Signaling")

DotPlot(ILC.BPC_CNA, features = rev(ILC_BPC_M7), idents = c("Areg ILC2", "IL-13 ILC2", "IL-10 ILC2", "Quiescent ILC2", "KLRG1 ILC2", "LTB ILC2"), group.by = "cell_type", split.by = "treatment", assay = "RNA", cols = "RdBu") + theme(axis.title = element_blank(), legend.title = element_text(size = 8, face ="bold"), legend.text = element_text(size = 8), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) + ggtitle("M7: Neuclear Receptors")

DotPlot(ILC.BPC_CNA, features = rev(ILC_BPC_M8), idents = c("Areg ILC2", "IL-13 ILC2", "IL-10 ILC2", "Quiescent ILC2", "KLRG1 ILC2", "LTB ILC2"), group.by = "cell_type", split.by = "treatment", assay = "RNA", cols = "RdBu") + theme(axis.title = element_blank(), legend.title = element_text(size = 8, face ="bold"), legend.text = element_text(size = 8), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) + ggtitle("M8: Electron Transport Chain")

DotPlot(ILC.BPC_CNA, features = rev(ILC_BPC_M9), idents = c("Areg ILC2", "IL-13 ILC2", "IL-10 ILC2", "Quiescent ILC2", "KLRG1 ILC2", "LTB ILC2"), group.by = "cell_type", split.by = "treatment", assay = "RNA", cols = "RdBu") + theme(axis.title = element_blank(), legend.title = element_text(size = 8, face ="bold"), legend.text = element_text(size = 8), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) + ggtitle("M9: Cytoplasmic Ribosomal Proteins")
```

```{r ILC2 module expression for paper, fig.width = 11, fig.height = 5}
ILC2_M1_M3_M7 <- DotPlot(ILC.BPC_CNA, features = Module_M1_M3_M7, idents = c("Areg ILC2", "IL-13 ILC2", "IL-10 ILC2", "Quiescent ILC2", "KLRG1 ILC2", "LTB ILC2"), group.by = "cell_type", split.by = "treatment", assay = "RNA", cols = "RdBu") + theme(axis.title = element_blank(), legend.title = element_text(size = 8, face ="bold"), legend.text = element_text(size = 8), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1), axis.text.y = element_text(color = c("dimgray", "black"))) + ggtitle("M1: TYROBP causal Network/M3: MAPK signaling pathway/M7: Neuclear Receptors")

ILC2_M1_M3_M7


ILC2_M2_M6 <- DotPlot(ILC.BPC_CNA, features = Module_M2_M6, idents = c("Areg ILC2", "IL-13 ILC2", "IL-10 ILC2", "Quiescent ILC2", "KLRG1 ILC2", "LTB ILC2"), group.by = "cell_type", split.by = "treatment", assay = "RNA", cols = "RdBu") + theme(axis.title = element_blank(), legend.title = element_text(size = 8, face ="bold"), legend.text = element_text(size = 8), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1), axis.text.y = element_text(color = c("dimgray", "black"))) + ggtitle("M2: DNA Replication/M6: p53 Signaling")

ILC2_M2_M6


ILC2_M8 <- DotPlot(ILC.BPC_CNA, features = Module_M8, idents = c("Areg ILC2", "IL-13 ILC2", "IL-10 ILC2", "Quiescent ILC2", "KLRG1 ILC2", "LTB ILC2"), group.by = "cell_type", split.by = "treatment", assay = "RNA", cols = "RdBu") + theme(axis.title = element_blank(), legend.title = element_text(size = 8, face ="bold"), legend.text = element_text(size = 8), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1), axis.text.y = element_text(color = c("dimgray", "black"))) + ggtitle("M8: Electron Transport Chain")

ILC2_M8


ILC2_M5_M9 <- DotPlot(ILC.BPC_CNA, features = Module_M5_M9, idents = c("Areg ILC2", "IL-13 ILC2", "IL-10 ILC2", "Quiescent ILC2", "KLRG1 ILC2", "LTB ILC2"), group.by = "cell_type", split.by = "treatment", assay = "RNA", cols = "RdBu") + theme(axis.title = element_blank(), legend.title = element_text(size = 8, face ="bold"), legend.text = element_text(size = 8), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1), axis.text.y = element_text(color = c("dimgray", "black"))) + ggtitle("M5/M9: Cytoplasmic Ribosomal Proteins")

ILC2_M5_M9


ILC2_M4 <- DotPlot(ILC.BPC_CNA, features = Module_M4, idents = c("Areg ILC2", "IL-13 ILC2", "IL-10 ILC2", "Quiescent ILC2", "KLRG1 ILC2", "LTB ILC2"), group.by = "cell_type", split.by = "treatment", assay = "RNA", cols = "RdBu") + theme(axis.title = element_blank(), legend.title = element_text(size = 8, face ="bold"), legend.text = element_text(size = 8), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1), axis.text.y = element_text(color = c("dimgray", "black"))) + ggtitle("M4: Type II interferon signaling (IFNG)")

ILC2_M4
```


# Oxidative stress gene sets

```{r Functional gene sets}
# Prooxidant genes
ProOx <- c("Cyba", "Hsp90aa1", "Hsp90ab1", "Hspa8")

# Antioxidant genes
AntiOx <- c("Gpx1", "Gpx4", "ndufa13", "Prdx1", "Prdx2", "Prdx3", "Prdx4", "Prdx5", "Prdx6", "Sod1")
```


# Oxidative stress gene expression in NK and ILC1

```{r NK_ILC1 Prooxidant genes, fig.width = 6, fig.height = 3}
NK_ILC1_ProOx <- DotPlot(ILC.BPC_CNA, features = c(ProOx), idents = c("NK", "ILC1", "ST-2 NK"), group.by = "cell_type", split.by = "treatment", assay = "RNA", cols = "RdBu") + theme(axis.title = element_blank(), legend.title = element_text(size = 8, face ="bold"), legend.text = element_text(size = 8), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1), axis.text.y = element_text(color = c("dimgray", "black"))) + ggtitle("Oxidative-stress-related genes")

NK_ILC1_ProOx
```

```{r NK_ILC1 Antioxidant genes, fig.width = 5, fig.height = 3}
NK_ILC1_AntiOx <- DotPlot(ILC.BPC_CNA, features = c(AntiOx), idents = c("NK", "ILC1", "ST-2 NK"), group.by = "cell_type", split.by = "treatment", assay = "RNA", cols = "RdBu") + theme(axis.title = element_blank(), legend.title = element_text(size = 8, face ="bold"), legend.text = element_text(size = 8), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1), axis.text.y = element_text(color = c("dimgray", "black"))) + ggtitle("Antioxidant genes")

NK_ILC1_AntiOx
```


# Oxidative stress gene expression in ILC2

```{r ILC2 Prooxidant genes, fig.width = 7, fig.height = 5}
ILC2_ProOx <- DotPlot(ILC.BPC_CNA, features = c(ProOx), idents = c("Areg ILC2", "IL-13 ILC2", "IL-10 ILC2", "Quiescent ILC2", "KLRG1 ILC2", "LTB ILC2"), group.by = "cell_type", split.by = "treatment", assay = "RNA", cols = "RdBu") + theme(axis.title = element_blank(), legend.title = element_text(size = 8, face ="bold"), legend.text = element_text(size = 8), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1), axis.text.y = element_text(color = c("dimgray", "black"))) + ggtitle("Oxidative-stress-related genes")

ILC2_ProOx
```

```{r ILC2 Antioxidant genes, fig.width = 6, fig.height = 5}
ILC2_AntiOx <- DotPlot(ILC.BPC_CNA, features = c(AntiOx), idents = c("Areg ILC2", "IL-13 ILC2", "IL-10 ILC2", "Quiescent ILC2", "KLRG1 ILC2", "LTB ILC2"), group.by = "cell_type", split.by = "treatment", assay = "RNA", cols = "RdBu") + theme(axis.title = element_blank(), legend.title = element_text(size = 8, face ="bold"), legend.text = element_text(size = 8), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1), axis.text.y = element_text(color = c("dimgray", "black"))) + ggtitle("Antioxidant genes")

ILC2_AntiOx
```


# ILC subsets

```{r ILC subsets}
# NK
NKC <- Thy1 %>%
  subset(idents = c("NK", "ST-2 NK"), invert = FALSE) %>%
  SetIdent(value = "treatment")

table(NKC@meta.data$cell_type, split.by = NKC@meta.data$treatment)

# ILC1
ILC1 <- Thy1 %>% 
  subset(idents = c("ILC1"), invert = FALSE) %>%
  SetIdent(value = "treatment")

table(ILC1@meta.data$cell_type, split.by = ILC1@meta.data$treatment)

# ILC2
ILC2 <- Thy1 %>% 
  subset(idents = c("Areg ILC2", "IL-13 ILC2", "IL-10 ILC2", "Quiescent ILC2", "KLRG1 ILC2", "LTB ILC2"), invert = FALSE)

table(ILC2@meta.data$cell_type, split.by = ILC2@meta.data$treatment)
```


# Packages for differential expression with volcano plot and enrichment network

```{r Load the packages for enrichment network}
# Load the package (clusterProfiler)
library(clusterProfiler)

# Set the desired organism below
# See all annotations available here: http://bioconductor.org/packages/release/BiocViews.html#___OrgDb
organism = "org.Mm.eg.db"
#BiocManager::install(organism, character.only = TRUE)
library(organism, character.only = TRUE)

# Load the package to do enrichment map (enrichplot)
library(enrichplot)

# Load the package to turn the first letter uppercase with str_to_title (stringr)
library(stringr)
```

```{r Load the package for volcano plot}
# Load the package for volcano plot
library(EnhancedVolcano)
```


# NK gene set enrichment analysis

```{r NKC differential expression}
# Find the differential expressed genes in NKC
NKC_differential <- FindMarkers(NKC, ident.1 = "Alt.BPC", ident.2 = "Alt", log2FC.threshold = -Inf, test.use = "wilcox", only.pos = FALSE, assay = "RNA", slot = "scale.data")

head(NKC_differential)

NKC_differential <- NKC_differential[order(abs(NKC_differential$avg_diff), decreasing = T), ]

# Find the top 20 differential expression in NKC
NKC_differential %>%
  top_n(-20) -> top_NKC_differential

head(top_NKC_differential)
```

```{r NKC DE Volcano plot, fig.width = 6, fig.height = 6}
# Plot the top 10 differential genes with volcano plot
NKC_volcano <- EnhancedVolcano(NKC_differential,
    lab = rownames(NKC_differential),
    selectLab = rownames(top_NKC_differential),
    x = 'avg_diff',
    y = 'p_val_adj',
    title = 'NK Alt.BPC vs. Alt',
    pCutoff = 10e-5,
    FCcutoff = 0.5,
    pointSize = 3.0,
    labSize = 6.0) +
  labs(subtitle = NULL)

NKC_volcano
```

```{r NKC GO_Cellular_Component_2023 GSEA, fig.width = 12, fig.height = 4}
NKC_gse_bar <- # Show the up- and down-regulated pathways
DEenrichRPlot(NKC, ident.1 = "Alt.BPC", ident.2 = "Alt", balanced = TRUE, logfc.threshold = 0.25, assay = "RNA", max.genes = 2000, test.use = "wilcox",p.val.cutoff = 0.05, enrich.database = "GO_Cellular_Component_2023", num.pathway = 5, return.gene.list = FALSE)

NKC_gse_bar
```

```{r NKC GSEA map, fig.height = 16, fig.width = 16}
# Convert gene label from SYMBOL to ENTREZID
NKC_gene_id <- bitr(rownames(NKC_differential), fromType="SYMBOL", toType="ENTREZID", OrgDb="org.Mm.eg.db")

# Create a gene rank based on the gene expression fold change
NKC_gene_list <- setNames(NKC_differential$avg_diff, NKC_gene_id$ENTREZID)

head(NKC_gene_list)

# Omit any NA values 
NKC_gene_rank<-na.omit(NKC_gene_list)

# Sort the list in decreasing order (required for clusterProfiler)
NKC_gene_rank = sort(NKC_gene_rank, decreasing = TRUE)

head(NKC_gene_rank)

# Perform GSE analysis
NKC_gse <- gseGO(geneList      = NKC_gene_rank,
                       ont           = "CC", 
                       keyType       = "ENTREZID", 
                       nPerm         = 10000, 
                       minGSSize     = 10, 
                       maxGSSize     = 1000, 
                       pvalueCutoff  = 0.05, 
                       verbose       = TRUE, 
                       OrgDb         = "org.Mm.eg.db", 
                       pAdjustMethod = "none",
                       by = "fgsea")
# ont setting: one of "BP", "MF", and "CC" subontologies, or "ALL" for all three
# Biological Process (BP):  	biological_process [GO:0008150] <30>
# Cellular Component (CC):  	cellular_component [GO:0005575] <22>
# Molecular Function (MF):  	molecular_function [GO:0003674] <15>


# Convert gene ID to Symbol
NKC_gse <- setReadable(NKC_gse, 'org.Mm.eg.db', 'ENTREZID')

# CategorySize can be either 'pvalue' or 'geneNum'
NKC_gse_map <- cnetplot(NKC_gse, categorySize="pvalue", foldChange=NKC_gene_rank, colorEdge = TRUE, showCategory = 5)

NKC_gse_map
```

```{r NKC GSEA plots, fig.height = 6, fig.width = 8}
# Use the `Gene Set` param for the index in the title, and as the value for geneSetId
gseaplot(NKC_gse, by = "all", title = NKC_gse$Description[1], geneSetID = 1)
gseaplot(NKC_gse, by = "all", title = NKC_gse$Description[2], geneSetID = 2)
gseaplot(NKC_gse, by = "all", title = NKC_gse$Description[3], geneSetID = 3)
gseaplot(NKC_gse, by = "all", title = NKC_gse$Description[4], geneSetID = 4)
gseaplot(NKC_gse, by = "all", title = NKC_gse$Description[5], geneSetID = 5)
```


# ILC1 gene set enrichment and analysis

```{r ILC1 differential expression}
# Find the differential expressed genes in ILC1
ILC1_differential <- FindMarkers(ILC1, ident.1 = "Alt.BPC", ident.2 = "Alt", log2FC.threshold = -Inf, test.use = "wilcox", only.pos = FALSE, assay = "RNA", slot = "scale.data")

head(ILC1_differential)

ILC1_differential <- ILC1_differential[order(abs(ILC1_differential$avg_diff), decreasing = T), ]

# Find the top 20 differential expression in ILC1
ILC1_differential %>%
  top_n(-20) -> top_ILC1_differential

head(top_ILC1_differential)
```

```{r ILC1 DE Volcano plot, fig.width = 6, fig.height = 6}
# Plot the top 10 differential genes with volcano plot
ILC1_volcano <- EnhancedVolcano(ILC1_differential,
    lab = rownames(ILC1_differential),
    selectLab = rownames(top_ILC1_differential),
    x = 'avg_diff',
    y = 'p_val_adj',
    title = 'ILC1 Alt.BPC vs. Alt',
    pCutoff = 10e-5,
    FCcutoff = 0.5,
    pointSize = 3.0,
    labSize = 6.0) +
  labs(subtitle = NULL)

ILC1_volcano
```

```{r ILC1 GO_Cellular_Component_2023 GSEA, fig.width = 12, fig.height = 4}
# Show the up- and down-regulated pathways
ILC1_gse_bar <- DEenrichRPlot(ILC1, ident.1 = "Alt.BPC", ident.2 = "Alt", balanced = TRUE, logfc.threshold = 0.25, assay = "RNA", max.genes = 2000, test.use = "wilcox",p.val.cutoff = 0.05, enrich.database = "GO_Cellular_Component_2023", num.pathway = 5, return.gene.list = FALSE)

ILC1_gse_bar
```

```{r ILC1 GSEA map, fig.height = 16, fig.width = 16}
# Convert gene label from SYMBOL to ENTREZID
ILC1_gene_id <- bitr(rownames(ILC1_differential), fromType="SYMBOL", toType="ENTREZID", OrgDb="org.Mm.eg.db")

# Create a gene rank based on the gene expression fold change
ILC1_gene_list <- setNames(ILC1_differential$avg_diff, ILC1_gene_id$ENTREZID)

head(ILC1_gene_list)

# Omit any NA values 
ILC1_gene_rank<-na.omit(ILC1_gene_list)

# Sort the list in decreasing order (required for clusterProfiler)
ILC1_gene_rank = sort(ILC1_gene_rank, decreasing = TRUE)

head(ILC1_gene_rank)

# Perform GSE analysis
ILC1_gse <- gseGO(geneList      = ILC1_gene_rank,
                       ont           = "CC", 
                       keyType       = "ENTREZID", 
                       nPerm         = 10000, 
                       minGSSize     = 10, 
                       maxGSSize     = 1000, 
                       pvalueCutoff  = 0.05, 
                       verbose       = TRUE, 
                       OrgDb         = "org.Mm.eg.db", 
                       pAdjustMethod = "none",
                       by = "fgsea")
# ont setting: one of "BP", "MF", and "CC" subontologies, or "ALL" for all three
# Biological Process (BP):  	biological_process [GO:0008150] <30>
# Cellular Component (CC):  	cellular_component [GO:0005575] <22>
# Molecular Function (MF):  	molecular_function [GO:0003674] <15>


# Convert gene ID to Symbol
ILC1_gse <- setReadable(ILC1_gse, 'org.Mm.eg.db', 'ENTREZID')

# CategorySize can be either 'pvalue' or 'geneNum'
ILC1_gse_map <- cnetplot(ILC1_gse, categorySize="pvalue", foldChange=ILC1_gene_rank, colorEdge = TRUE, showCategory = 5)

ILC1_gse_map
```

```{r ILC1 GSEA plots, fig.height = 6, fig.width = 8}
# Use the `Gene Set` param for the index in the title, and as the value for geneSetId
gseaplot(ILC1_gse, by = "all", title = ILC1_gse$Description[1], geneSetID = 1)
gseaplot(ILC1_gse, by = "all", title = ILC1_gse$Description[2], geneSetID = 2)
gseaplot(ILC1_gse, by = "all", title = ILC1_gse$Description[3], geneSetID = 3)
gseaplot(ILC1_gse, by = "all", title = ILC1_gse$Description[4], geneSetID = 4)
gseaplot(ILC1_gse, by = "all", title = ILC1_gse$Description[5], geneSetID = 5)
```


# ILC2 gene set enrichment and analysis

```{r ILC2 differential expression}
ILC2 <- SetIdent(ILC2, value = "treatment")

# Find the differential expressed genes in ILC2
ILC2_differential <- FindMarkers(ILC2, ident.1 = "Alt.BPC", ident.2 = "Alt", log2FC.threshold = -Inf, test.use = "wilcox", only.pos = FALSE, assay = "RNA", slot = "scale.data")

head(ILC2_differential)

ILC2_differential <- ILC2_differential[order(abs(ILC2_differential$avg_diff), decreasing = T), ]

# Find the top 20 differential expression in ILC2
ILC2_differential %>%
  top_n(-20) -> top_ILC2_differential

head(top_ILC2_differential)
```

```{r ILC2 DE Volcano plot, fig.width = 6, fig.height = 6}
# Plot the top 10 differential genes with volcano plot
ILC2_volcano <- EnhancedVolcano(ILC2_differential,
    lab = rownames(ILC2_differential),
    selectLab = rownames(top_ILC2_differential),
    x = 'avg_diff',
    y = 'p_val_adj',
    title = 'ILC2 Alt.BPC vs. Alt',
    pCutoff = 10e-32,
    FCcutoff = 0.5,
    pointSize = 3.0,
    labSize = 6.0) +
  labs(subtitle = NULL)

ILC2_volcano
```

```{r ILC2 GO_Cellular_Component_2023 GSEA, fig.width = 12, fig.height = 4}
# Show the up- and down-regulated pathways
ILC2_gse_bar <- DEenrichRPlot(ILC2, ident.1 = "Alt.BPC", ident.2 = "Alt", balanced = TRUE, logfc.threshold = 0.25, assay = "RNA", max.genes = 2000, test.use = "wilcox",p.val.cutoff = 0.05, enrich.database = "GO_Cellular_Component_2023", num.pathway = 5, return.gene.list = FALSE)

ILC2_gse_bar
```

```{r ILC2 GSEA map, fig.height = 16, fig.width = 16}
# Convert gene label from SYMBOL to ENTREZID
ILC2_gene_id <- bitr(rownames(ILC2_differential), fromType="SYMBOL", toType="ENTREZID", OrgDb="org.Mm.eg.db")

# Create a gene rank based on the gene expression fold change
ILC2_gene_list <- setNames(ILC2_differential$avg_diff, ILC2_gene_id$ENTREZID)

head(ILC2_gene_list)

# Omit any NA values 
ILC2_gene_rank<-na.omit(ILC2_gene_list)

# Sort the list in decreasing order (required for clusterProfiler)
ILC2_gene_rank = sort(ILC2_gene_rank, decreasing = TRUE)

head(ILC2_gene_rank)

# Perform GSE analysis
ILC2_gse <- gseGO(geneList      = ILC2_gene_rank,
                       ont           = "CC", 
                       keyType       = "ENTREZID", 
                       nPerm         = 10000, 
                       minGSSize     = 10, 
                       maxGSSize     = 1000, 
                       pvalueCutoff  = 0.05, 
                       verbose       = TRUE, 
                       OrgDb         = "org.Mm.eg.db", 
                       pAdjustMethod = "none",
                       by = "fgsea")
# ont setting: one of "BP", "MF", and "CC" subontologies, or "ALL" for all three
# Biological Process (BP):  	biological_process [GO:0008150] <30>
# Cellular Component (CC):  	cellular_component [GO:0005575] <22>
# Molecular Function (MF):  	molecular_function [GO:0003674] <15>


# Convert gene ID to Symbol
ILC2_gse <- setReadable(ILC2_gse, 'org.Mm.eg.db', 'ENTREZID')

# CategorySize can be either 'pvalue' or 'geneNum'
ILC2_gse_map <- cnetplot(ILC2_gse, categorySize="pvalue", foldChange=ILC2_gene_rank, colorEdge = TRUE, showCategory = 5)

ILC2_gse_map
```

```{r ILC2 GSEA plots, fig.height = 6, fig.width = 8}
# Use the `Gene Set` param for the index in the title, and as the value for geneSetId
gseaplot(ILC2_gse, by = "all", title = ILC2_gse$Description[1], geneSetID = 1)
gseaplot(ILC2_gse, by = "all", title = ILC2_gse$Description[2], geneSetID = 2)
gseaplot(ILC2_gse, by = "all", title = ILC2_gse$Description[3], geneSetID = 3)
gseaplot(ILC2_gse, by = "all", title = ILC2_gse$Description[4], geneSetID = 4)
gseaplot(ILC2_gse, by = "all", title = ILC2_gse$Description[5], geneSetID = 5)
```


# Data oupput

```{r ILC_BPC figure for paper, fig.width = 12, fig.height = 15}
# Generate the plot in paper figure form
figure4_1 <- plot_grid(NULL, ILC.PlotKMEs, labels = c('(A)'), label_size = 18, nrow = 1, ncol = 2, rel_widths = c(0.2, 2))
figure4_2 <- plot_grid(ILC.EnrichrDotPlot, labels = c('(B)'), label_size = 18, ncol = 1)
figure4_3 <- plot_grid(NULL, NULL, NULL, NULL, labels = c('(C)', 'ILC1 and NK', '(D)', 'ILC2'), label_size = 18, nrow = 1, rel_widths = c(0.55, 1.45, 0.87, 1.13))
figure4_4 <- plot_grid(ILC1_NK_OverlapBarPlot, ILC2_OverlapBarPlot)

figure4 <- plot_grid(figure4_1, figure4_2, figure4_3, figure4_4,
                     nrow = 4,
                     ncol = 1, 
                     rel_heights = c(1.4, 1, 0.1, 0.6)
                     )

figure4
```

```{r Module expression figure for paper, fig.width = 16, fig.height = 9}
# Generate the plot in paper figure form
figure5_1 <- plot_grid(NULL, NK_ILC1_module, NULL, NK_ILC1_ProOx, NULL, NK_ILC1_AntiOx, labels = c('(A)', ' ', '(C)', ' ', '(E)'), label_size = 18, nrow = 1, rel_widths = c(0.1, 1, 0.1, 1, 0.1, 1))
figure5_2 <- plot_grid(ILC2_module, ILC2_ProOx, ILC2_AntiOx, labels = c('(B)', '(D)', '(F)'), label_size = 18, nrow = 1, rel_widths = c(1, 1, 1))

figure5 <- plot_grid(figure5_1, figure5_2,
  nrow = 2,
  ncol = 1,
  rel_heights = c(1, 1.2)
  )

figure5
```

```{r Ribosomal protein for paper, fig.width = 20, fig.height = 22}
# Generate the plot in paper figure form
figure6_1 <- plot_grid(NKC_volcano, NKC_gse_map, labels = c('(A)', '(D)'), label_size = 18, nrow = 1, rel_widths = c(1, 1.8))
figure6_2 <- plot_grid(ILC1_volcano, ILC1_gse_map, labels = c('(B)', '(E)'), label_size = 18, nrow = 1, rel_widths = c(1, 1.8))
figure6_3 <- plot_grid(ILC2_volcano, ILC2_gse_map, labels = c('(C)', '(F)'), label_size = 18, nrow = 1, rel_widths = c(1, 1.8))

figure6 <- plot_grid(figure6_1, figure6_2, figure6_3,
  nrow = 3,
  ncol = 1,
  rel_heights = c(1, 1, 1)
  )

figure6
```

```{r NK_ILC1 figure for paper, fig.width = 14, fig.height = 12}
# Generate the plot in paper figure form
figureS2_1 <- plot_grid(NULL, NK_ILC1_M1_M3_M7, labels = c('(A)'), label_size = 18, nrow = 1, rel_widths = c(0.01, 2.3))
figureS2_2 <- plot_grid(NK_ILC1_M2_M6, NK_ILC1_M8, labels = c('(B)', '(C)'), label_size = 18, nrow = 1, rel_widths = c(1.4, 1))
figureS2_3 <- plot_grid(NK_ILC1_M4, NK_ILC1_M5_M9, labels = c('(D)', '(E)'), label_size = 18, nrow = 1, rel_widths = c(1, 1.5))

figureS2 <- plot_grid(figureS2_1, figureS2_2, figureS2_3,
  nrow = 3,
  ncol = 1,
  rel_heights = c(1, 1.1, 1)
  )

figureS2
```

```{r ILC2 figure for paper, fig.width = 14, fig.height = 14}
# Generate the plot in paper figure form
figureS3_1 <- plot_grid(NULL, ILC2_M1_M3_M7, labels = c('(A)'), label_size = 18, nrow = 1, rel_widths = c(0.01, 2.3))
figureS3_2 <- plot_grid(ILC2_M2_M6, ILC2_M8, labels = c('(B)', '(C)'), label_size = 18, nrow = 1, rel_widths = c(1.3, 1))
figureS3_3 <- plot_grid(ILC2_M4, ILC2_M5_M9, labels = c('(D)', '(E)'), label_size = 18, nrow = 1, rel_widths = c(1, 1.3))

figureS3 <- plot_grid(figureS3_1, figureS3_2, figureS3_3,
  nrow = 3,
  ncol = 1,
  rel_heights = c(1, 1.1, 1.1)
  )

figureS3
```


# Data save

```{r Output the figures}
# Create a folder for the figures
dir.create("./Figures", showWarnings = FALSE)

# Save the figure
ggsave2(plot = figure4, path = "./Figures", "Figure4.tiff", device = "tiff", dpi = 300, bg = "white", width = 12, height = 15, units = "in")
ggsave2(plot = figure5, path = "./Figures", "Figure5.tiff", device = "tiff", dpi = 300, bg = "white", width = 16, height = 9, units = "in")
ggsave2(plot = figure6, path = "./Figures", "Figure6.tiff", device = "tiff", dpi = 300, bg = "white", width = 20, height = 22, units = "in")
ggsave2(plot = figureS2, path = "./Figures", "FigureS2.tiff", device = "tiff", dpi = 300, bg = "white", width = 14, height = 12, units = "in")
ggsave2(plot = figureS3, path = "./Figures", "FigureS3.tiff", device = "tiff", dpi = 300, bg = "white", width = 14, height = 14, units = "in")
```

```{r Output the CSV files}
# Create a folder for CSV files
dir.create("./CSV", showWarnings = FALSE)

# Export to csv
write.csv(modules, file.path("./CSV", "ILC.BPC_hdWGCNA_modules.csv"))

write.csv(NKC_differential, file.path("./CSV", "NKC_differential.csv"))
write.csv(NKC_gse, file.path("./CSV", "NKC_gse.csv"))

write.csv(ILC1_differential, file.path("./CSV", "ILC1_differential.csv"))
write.csv(ILC1_gse, file.path("./CSV", "ILC1_gse.csv"))

write.csv(ILC2_differential, file.path("./CSV", "ILC2_differential.csv"))
write.csv(ILC2_gse, file.path("./CSV", "ILC2_gse.csv"))
```


<details>
  <summary>**Session Info**</summary>
```{r}
sessionInfo()
```

